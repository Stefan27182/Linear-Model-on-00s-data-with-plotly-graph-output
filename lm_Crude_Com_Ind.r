  ...
  for(m in 2:ncol(Shenzhen))
  {
    ...
    { 
      ...
      for(h in 1:5)
      {
        ...
        Shenzhen[i,m] <- ((mean(unlist(vec_1), na.rm = TRUE) + mean(unlist(vec_2), na.rm = TRUE))/2)
      } # end inner for
    } # end na if
  } # end column cycle for
} # end row cycle for
...
if(testr > 0)
{
  ...
  Shenzhen$Volume <- post_mice$Volume
...
#plot(Shenzhen$Date, Shenzhen$Close)
...
lm_frame <- cbind(Crude_02_through_05$Close, DJIA_02_through_05$Close, E_Mini_02_through_05$Close, Silver_02_through_05$Close, Shenzhen_02_through_05$Close, TSE_02_through_05$Close)

colnames(lm_frame) <- c("Crude", "DJIA", "E_Mini (S&P 500)", "Silver", "Shenzhen", "TSE")

lm_frame_df <- as.data.frame(lm_frame)

View(head(lm_frame_df))

#-----#

for(i in 1:nrow(lm_frame_df)) # replace dashes and null's with NA
{
  for(m in 1:ncol(lm_frame_df))
  {
    if(is.na(lm_frame_df[i,m]) == FALSE)
    {  
      if(as.character(lm_frame_df[i,m]) == "-" || as.character(lm_frame_df[i,m]) == "null")
      {
        lm_frame_df[i,m] <- NA
      } # end if
    }
...
for(i in 6:(nrow(lm_frame_df)-5)) # replace NA's w a moving average 5x5 (will miss first and last five rows in data frame, which must then be handled some other way e.g. MICE)
{
  for(m in 2:ncol(lm_frame_df))
  {
    if(is.na(lm_frame_df[i,m]) == TRUE)
    { 
      vec_1 <- vector(length = 5)
      vec_2 <- vector(length = 5)
      for(h in 1:5)
      {
        vec_1[h] <- lm_frame_df[(i+h), m]
        vec_2[h] <- lm_frame_df[(i-h), m]
        lm_frame_df[i,m] <- ((mean(unlist(vec_1), na.rm = TRUE) + mean(unlist(vec_2), na.rm = TRUE))/2)
      } # end inner for
    } # end na if
  } # end column cycle for
} # end row cycle for
...
Com_ind_test.lm <- lm(formula = E_Mini_02_through_05$Close ~ DJIA_02_through_05$Close + TSE_02_through_05$Close+ Crude_02_through_05$Close+ Silver_02_through_05$Close+ Shenzhen_02_through_05$Close, data = lm_frame_df)

summary(Com_ind_test.lm)

Com_ind_.lm <- lm(formula = E_Mini_02_through_05$Close ~ DJIA_02_through_05$Close + TSE_02_through_05$Close + Shenzhen_02_through_05$Close, data = lm_frame_df)

summary(Com_ind_.lm)

new_lm_frame_df <- matrix(nrow = nrow(TSE_02_through_05), ncol = 7)

new_lm_frame_df <- as.data.frame(new_lm_frame_df)
 
new_lm_frame_df[ , 1] <- c(DJIA_02_through_05$Date)

colnames(new_lm_frame_df) <- c("Date", "Crude", "DJIA", "E_Mini", "Silver", "Shenzhen", "TSE") 

new_lm_frame_df[,2:7] <- lm_frame_df[,1:6]
...
plot_grid(E_Mini_plot, DJIA_plot, TSE_plot, TSE_DJIA_intersection, labels = c('E_Mini (S&P 500)', '          DJIA', '          TSE', 'DJIA & TSE Crossover'))

Shenzhen_plot <- ggplot(new_lm_frame_df, aes(Date)) +
  geom_line(aes(y = Shenzhen_02_through_05$Close), colour = "blueviolet") + ylab("Shenzhen in Renminbi ¥")
